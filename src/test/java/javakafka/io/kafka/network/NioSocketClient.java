package javakafka.io.kafka.network;
/**
 * @author tf
 * @version 创建时间：2019年1月23日 上午10:36:16
 * @ClassName 类名称
 */
...
public class NioSocketClient extends Thread {
	...
	public void send(SelectionKey key) throws InterruptedException {
		SocketChannel channel = (SocketChannel) key.channel();
		...
		for (int i = 0; i < 10; i++) {
			...
			while (byteBuffer.hasRemaining()) {
				try {
					ByteBuffer headBuffer = ByteBuffer.allocate(4).putInt(ss.getBytes().length);
					headBuffer.rewind();
